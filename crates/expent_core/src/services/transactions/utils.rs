    ...
    if let Some(old) = old_txn {
        ...
        if old_is_active {
            if let Some(sw_id) = &old.source_wallet_id {
                crate::wallets::ops::adjust_balance(db, sw_id, old.amount, true).await?;
            }
            if let Some(dw_id) = &old.destination_wallet_id {
                crate::wallets::ops::adjust_balance(db, dw_id, -old.amount, true).await?;
            }
        }
    }
    ...
    if let Some(new) = new_txn {
        ...
        if new_is_active {
            if let Some(sw_id) = &new.source_wallet_id {
                crate::wallets::ops::adjust_balance(db, sw_id, -new.amount, true).await?;
            }
            if let Some(dw_id) = &new.destination_wallet_id {
                crate::wallets::ops::adjust_balance(db, dw_id, new.amount, true).await?;
            }
        }
    }
