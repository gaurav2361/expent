...
import { AlertCircle, Calendar, ChevronLeft, CreditCard, MoreHorizontal, Sparkles } from "lucide-react-native";
                ...
                >
                  <View className="flex-row items-center justify-between">
                    <View className="flex-row items-center gap-4">
                      <View
                        className={`w-14 h-14 rounded-2xl items-center justify-center ${isDark ? "bg-white/20" : "bg-muted/50"}`}
                      >
                        <Text className="text-2xl">{sub.icon}</Text>
                      </View>
                      <View>
                        <Text
                          className={`font-bold text-xl tracking-tight ${isDark ? "text-primary-foreground" : "text-foreground"}`}
                        >
                          {sub.name}
                        </Text>
                    ...
                    <View className="items-end">
                      ...
                      <Text
                        className={`text-[10px] font-bold uppercase tracking-wider ${isDark ? "text-primary-foreground/60" : "text-muted-foreground"}`}
                      >
                        {sub.period}
                      </Text>
                    </View>
                    ...
                    <Animated.View entering={FadeIn} className="mt-8 gap-6 pt-6 border-t border-white/10">
                      <View className="flex-row items-center justify-between">
                        <View className="gap-1">
                          <Text
                            className={`${isDark ? "text-primary-foreground/60" : "text-muted-foreground"} text-[10px] font-bold uppercase tracking-widest`}
                          >
                            Payment Method
                          </Text>
                        ...
                        <Button
                          variant="outline"
                          className={`rounded-xl h-10 px-4 border-white/20 ${isDark ? "bg-white/10" : ""}`}
                          onPress={handlePress}
                        >
                          <Text
                            className={`text-xs font-bold ${isDark ? "text-primary-foreground" : "text-foreground"}`}
                          >
                            Manage
                          </Text>
                        </Button>
                      ...
                      <View className="flex-row items-center justify-between">
                        ...
                        <View
                          className={`flex-row items-center gap-1 px-2 py-1 rounded-lg ${isDark ? "bg-white/10" : "bg-primary/5"}`}
                        >
                          <AlertCircle size={12} color={isDark ? "white" : "hsl(var(--primary))"} />
                          <Text
                            className={`text-[10px] font-bold ${isDark ? "text-primary-foreground" : "text-primary"}`}
                          >
                            Auto-pay on
                          </Text>
                        </View>
                      ...
                      <Button
                        className={`rounded-2xl py-4 mt-2 h-14 ${isDark ? "bg-white" : "bg-primary"}`}
                        onPress={handlePress}
                      >
                        <Text className={`font-bold text-base ${isDark ? "text-primary" : "text-primary-foreground"}`}>
                          Cancel Subscription
                        </Text>
                      </Button>
                    </Animated.View>
