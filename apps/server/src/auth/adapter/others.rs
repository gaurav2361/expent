...
use better_auth::types_mod::{
    ...
    CreateOrganization, CreatePasskey, CreateTwoFactor, Invitation, InvitationOps,
    InvitationStatus, MemberOps, OrganizationOps, Passkey, PasskeyOps, TwoFactor, TwoFactorOps,
    UpdateApiKey, UpdateOrganization,
};
...
impl MemberOps for SqliteAdapter {
    ...
    async fn get_member(&self, _org_id: &str, _user_id: &str) -> AuthResult<Option<Self::Member>> {
        Ok(None)
    }
    ...
    async fn list_organization_members(&self, _org_id: &str) -> AuthResult<Vec<Self::Member>> {
        Ok(vec![])
    }
    async fn count_organization_members(&self, _org_id: &str) -> AuthResult<usize> {
        Ok(0)
    }
    async fn count_organization_owners(&self, _org_id: &str) -> AuthResult<usize> {
        Ok(0)
    }
}
