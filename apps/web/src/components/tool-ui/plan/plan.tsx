...
function areTodoPropsEqual(prev: PlanTodoItemProps, next: PlanTodoItemProps): boolean {
  ...
  return (
    prevStyle.animationDelay === nextStyle.animationDelay && prevStyle.animationFillMode === nextStyle.animationFillMode
  );
}
...
const PlanTodoItem = memo(function PlanTodoItem({ todo, className, style, showConnector }: PlanTodoItemProps) {
  ...
  if (!todo.description) {
    return (
      <li
        className={cn("relative -mx-2 flex cursor-default items-start gap-3 rounded-md px-2 py-1.5", className)}
        style={style}
      >
        {showConnector && (
          <div
