...
|---------------|-----------------|---------------------|
| `m20220101_000001` | **Base Architecture** | Instantiate foundational entities: `users`, `sessions`, `accounts`, `verifications`, `files`, `transactions`, `contact_links`, `p2p_requests`, `purchase_items`, `subscriptions`. |
| `m20260331_092335` | **Groups Extrusion** | Devolves the standalone `groups` and mapping `group_members` bridging tables. |
...
3. **Rust Type Alignment**: After applying an `.rs` migration block, the `crates/db/src/entities/` block *MUST* instantly be re-synced using sea-orm-cli mapping schemas to actual structs natively avoiding panic states.
